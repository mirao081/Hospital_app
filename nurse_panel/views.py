...
def nurse_dashboard(request):
    # Query medication orders, logs, and requests
    medication_orders = MedicationOrder.objects.select_related('patient', 'prescribed_by').all()
    medication_logs = MedicationLog.objects.select_related('order').order_by('-administered_at')[:10]
    med_requests = MedicationRequest.objects.order_by('-requested_at')[:10]

    # Initialize forms
    log_form = MedicationLogForm()
    req_form = MedicationRequestForm()

    # Handle POST requests
    if request.method == 'POST':
        if 'log_dose' in request.POST:
            log_form = MedicationLogForm(request.POST)
            if log_form.is_valid():
                log_form.save()
                return redirect('nurse_panel:nurse_dashboard')  # Change this to your actual dashboard URL name

        elif 'med_request' in request.POST:
            req_form = MedicationRequestForm(request.POST)
